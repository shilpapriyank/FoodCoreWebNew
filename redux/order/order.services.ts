...
import { CardPayment, OrderAddress } from "../../src/types/order-types/order.types";
...
export class OrderServices {
    ...
    static async getOrderInfo(restaurantId: number, locationId: number, orderId: number, customerId: number) {
        responseclass = new ResponseModel();
        ...
        responseclass = await handleAxiosPostAsync(data, orderInfo, methodName, true, restaurantId);
        if (responseclass.result != null && responseclass.status === API_RESPONSE_STATUS.SUCCESS) {
            return responseclass.result;
        } else {
            return null;
    ...
    static async repeatOrder(
        ...
        responseclass = await handleAxiosPostAsync(data, orderPlace, methodName, true, restaurantId);
        return responseclass;
    };

    static async placeOrder(order: any, restaurantId: number) {
        responseclass = new ResponseModel();
    ...
    static async getOrderTime(restaurantId: number, locationId: number, ordertype: string) {
        responseclass = new ResponseModel();
    ...
    static async addOrder(placeOrder: any, restaurantId: number) {
        responseclass = new ResponseModel();
        ...
        responseclass = await handleAxiosPostAsync(data, addorderurl, methodName, true, restaurantId);
        if (responseclass.result != null && responseclass.status === API_RESPONSE_STATUS.SUCCESS) {
    ...
    static async addorders(placeOrder: any, restaurantId: number) {
        responseclass = new ResponseModel();
    ...
    static async getAllCountry() {
        ...
        responseclass = await handleAxiosPostAsync(data, countryurl, methodName, false, 0);
        if (responseclass.result != null && responseclass.status === API_RESPONSE_STATUS.SUCCESS) {
            return responseclass.result;
        } else {
            return responseclass;
    ...
    static async confirmStripePayment(cardPayment: CardPayment) {
        ...
        responseclass = await handleAxiosPostAsync(data, paymenturl, methodName, true, cardPayment.restaurantId);
        if (responseclass.result != null && responseclass.status === API_RESPONSE_STATUS.SUCCESS) {
    ...
    static async getOrderTiming(restaurantId: number, locationId: number, ordertype: string, obj: OrderAddress, requestId = "") {
        responseclass = new ResponseModel();
    ...
    static async getOrderDeliveryServiceDetails(restaurantId: number, locationId: number, orderId: number) {
        responseclass = new ResponseModel();
    ...
    static async getPaymentDetails(GUID: string) {
        responseclass = new ResponseModel();
        const methodName = "getPaymentDetails";
        const endPoint = ENDPOINTS.GET_PAYMENT_DETAILS;
        const data = { GUID };
        responseclass = await handleAxiosPostAsync(data, endPoint, methodName, false, 0);
        if (responseclass.result != null && responseclass.status === API_RESPONSE_STATUS.SUCCESS) {
            return responseclass.result;
        } else {
            return null;
    ...
    static async getOrderDetailsInfoByGUID(GUID: string) {
        responseclass = new ResponseModel();
    ...
    static async generateTimeSlot(restaurantId: number, locationId: number, ordertype: string, scheduleDateTime: string) {
        responseclass = new ResponseModel();
