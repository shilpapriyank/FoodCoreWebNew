...
export class CustomerServices {
    static async getCustomerPassword(restaurantId: number, locationId: number, customerId: number) {
        ...
        responseclass = await handleAxiosPostAsync(data, passwordurl, methodName, true, restaurantId);
    ...
    static async updateCustomerInfo(userInfo: UpdateCustomerInfoModel) {
        ...
        responseclass = await handleAxiosPostAsync(data, userInfoUrl, methodName, true, userInfo.restaurantId);
    ...
    static async userExists(data: { phoneNumber: string; restaurantId: number }) {
        ...
        responseclass = await handleAxiosPostAsync(data, userInfoUrl, methodName, true, data?.restaurantId);
        return responseclass;
    }
    ...
    static async handleForgotPasswordRequest(
        ...
        responseclass = await handleAxiosPostAsync(data, forgetpasswordUrl, methodName, true, restaurantId);
        if (responseclass.status === API_RESPONSE_STATUS.SUCCESS) {
            handleNotify(`Forget password link sent to ${responseclass.result?.emailId} successfully!`, ToasterPositions.TopRight, ToasterTypes.Success);
            return true;
        } else {
            handleNotify("Customer update failed!", ToasterPositions.TopRight, ToasterTypes.Error);
            return false;
    ...
    static async userResetPasswordValidToken(token: string, restaurantId: number) {
        responseclass = new ResponseModel();
        ...
        responseclass = await handleAxiosPostAsync(data, resettokenUrl, methodName, true, restaurantId);
        return responseclass;
    }

    static async userResetPasswordRequest(obj: ResetPasswordRequest): Promise<any> {
    ...
    static async updateVerifiedPhoneNumber(updatePhoneModel: UpdatePhoneModel) {
        ...
        responseclass = await handleAxiosPostAsync(data, endPoint, methodName, true, updatePhoneModel.restaurantId);
        if (responseclass.result != null && responseclass.status === API_RESPONSE_STATUS.SUCCESS) {
            return responseclass.result;
        } else {
            return null;
    ...
    static async checkExistingCustomerPhoneNumber(checkPhoneRequestModel: CheckPhoneRequestModel) {
        responseclass = new ResponseModel();
        ...
        responseclass = await handleAxiosPostAsync(data, endPoint, methodName, true, checkPhoneRequestModel.restaurantId);
        if (responseclass.result != null && responseclass.status === API_RESPONSE_STATUS.SUCCESS) {
            return responseclass;
        } else {
            return responseclass;
