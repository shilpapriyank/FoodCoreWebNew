...
const selectedDeliverySlice = createSlice({
  ...
  initialState,
  reducers: {
    [SelectedDeliveryTypes.SAVE_CHOOSE_TIME]: (
      state,
      action: PayloadAction<ChooseTime>
    ) => {
      state.choosetime = action.payload;
    },
    [SelectedDeliveryTypes.SET_PICKUP_OR_DELIVERY]: (
      state,
      action: PayloadAction<string>
    ) => {
      state.pickupordelivery = action.payload;
    },
    [SelectedDeliveryTypes.SELECTED_DELIVERY_ADDRESS]: (
      state,
