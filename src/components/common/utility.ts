...
export const handleDefaultDynamicFieldColor = (color = "#004aad", theme: string) => {
    ...
    if (themeDetail?.name === ThemeObj.dominos || themeDetail?.name === ThemeObj.newtheme) {
...
export const checkCheckoutDisable = (cartdata: any, pickupordelivery: any, dtotal: any) => {
    if (pickupordelivery === ORDERTYPE.Pickup || pickupordelivery === ORDERTYPE.Delivery) {
        ...
        let isAnydeliveryItemNotAvailable = cartItems?.some((item: any) => item?.categorydeliveryavailable === false)

        if ((pickupordelivery === ORDERTYPE.Pickup && isAnyPickupItemNotAvailable) || (pickupordelivery === ORDERTYPE.Delivery && isAnydeliveryItemNotAvailable) || checkMenuItemTimeAvailability(cartItems)) {
            return true;
        } else if (dtotal !== undefined && (pickupordelivery === ORDERTYPE.Delivery && dtotal === 0)) {
            return true;
        }
        // else{
        //     return false;
...
export const checkMenuItemAvailability = (categoryList: any, catId: any, pickupordelivery: any) => {
    ...
    if (pickupordelivery === ORDERTYPE.Pickup && filteredCategory.istakeoutavailable === false) {
        isItemAvailable = false;
    } else if (pickupordelivery === ORDERTYPE.Delivery && filteredCategory.isdeliveryavailable === false) {
        isItemAvailable = false;
    } else {
...
export const getAvailableCartRelativeData = (categoryList: any, pickupordelivery: any, cartRelativeData: any) => {
    ...
    if (pickupordelivery === "Pickup") {
        ...
        let categoryData = categoryList.filter((category: any) => {
            return cartRelativeData?.some((relativeItem: any) => (relativeItem?.categoryId === category?.catId && category?.istakeoutavailable))
        })
        ...
        let AvailableCartRelativeItem = cartRelativeData?.filter((relativeItem: any) => {
            return categoryData?.some((catItem: any) => catItem?.catId === relativeItem.categoryId)
        })
    ...
    if (pickupordelivery === ORDERTYPE.Delivery) {
        ...
        let categoryData = categoryList?.filter((category: any) => {
            return cartRelativeData?.some((relativeItem: any) => (relativeItem.categoryId === category?.catId && category?.isdeliveryavailable))
        })
...
export const orderDisable = (restaurantinfo: any, deliveryaddressinfo: any, restaurantWindowTime: any) => {
    ...
    } else if (restaurantinfo.isdelivery === false && restaurantinfo.istakeaway === false) {
        orderDisableObj.errormessage = location.orderingMessage;
        orderDisableObj.isorderdisable = true;
    } else if (!((location.istakeaway === true && pickupWindow && pickupWindow.length > 0) || (location.isdelivery === true && deliveryWindow && deliveryWindow.length > 0))) {
        orderDisableObj.errormessage = 'We are away for a bit.Check back soon.';
        orderDisableObj.isorderdisable = true;
    } else {
        if (
            deliveryaddressinfo &&
            deliveryaddressinfo.pickupordelivery === ORDERTYPE.Pickup && (location.isTakeoutOrderingDisable === true ||
                restaurantinfo.istakeaway === false || location.istakeaway === false)
        ) {
            orderDisableObj.errormessage = location.orderingMessage;
            orderDisableObj.isorderdisable = true;
        } else if (
            ...
            orderDisableObj.errormessage = location.isdelivery ? location.orderingMessage : "Delivery is not available";
            orderDisableObj.isorderdisable = true;
        }
    }
...
export const onLoadSetDefaultFlag = (defaultflag: any, countryList: any, locationCountry: any) => {
    if (defaultflag) {
        defaultflag.className = `iti-flag ${locationCountry.region}`
        document.querySelector(".selected-flag")?.setAttribute("title", `${locationCountry.title}: ${locationCountry.countryCode}`)
        for (let index = 0; index < countryList?.children?.length; index++) {
            const element = countryList?.children[index];
            if (element?.getAttribute("data-country-code") === locationCountry.region) {
...
export const handleSetDeliveryTypeError = (pickupordelivery: any, deliveryaddressinfo: any, carttotal: any, dcharges: any, cart: any, cartdata: any, isCartError = true) => {
    let errorMessage = "";
    if (pickupordelivery === ORDERTYPE.Delivery && (deliveryaddressinfo?.length === 0 || deliveryaddressinfo === null) && (carttotal?.cartCount > 0 || cart?.cartitemcount > 0)) {
    ...
    } else if (dcharges && parseInt(dcharges.isdelivery) === 0 && carttotal.subTotal < parseFloat(dcharges.minOrderForAddress) && carttotal?.cartCount > 0 && isCartError) {
        errorMessage =
            "Minimum order value for delivery at your address is " +
            carttotal.currencySymbol + dcharges.minOrderForAddress + " before tax. Please add some more items.";
    } else if (dcharges && parseInt(dcharges.isdelivery) === 0 && carttotal?.cartCount > 0 && isCartError) {
        if (dcharges?.returnMessage) {
            errorMessage = dcharges.returnMessage
        }
        else {
            if (parseFloat(dcharges.maxkms) > 0 && (dcharges.distance > parseFloat(dcharges.maxkms))) {
                errorMessage = "Sorry ! We do not deliver above " + dcharges.maxkms + " KM.";
            }
        }
    } else if (dcharges && dcharges.minOrderForAddress > cart.carttotal.subTotal && carttotal?.cartCount > 0 &&
        cartdata?.cartDetails?.cartItemDetails != undefined && cartdata?.cartDetails?.cartItemDetails?.length > 0 && isCartError) {
        errorMessage = "Minimum order value for delivery is " + carttotal.currencySymbol + dcharges.minOrderForAddress + " before tax. Please add some more items.";
    } else {
        errorMessage = "";
    }
    return errorMessage;
...
export const getCheckTimeArr = (orderTime: any, restaurantinfo: any, orderDate: any = "", isasap: any) => {
    let Time = []
...
export const sortArrayOnSelectedLocation = (addressList: any, defaultLocationId: any) => {
...
export const getLocationFromUrl = (locationUrl: any, locationList: any, defaultLocation: any) => {
    let locationDetail = {
    ...
    if (locationUrl === defaultLocation?.locationURL?.trim()) {
        ...
        let getUrlLocation = locationList?.find((location: any) => location?.locationURL === locationUrl)
...
export function getMenuItemdetailFormate(data: any) {
    ...
    data?.PriceList?.forEach((size: any) => {
        let toppingObj = {
            subparameterId: size?.subparameterId,
            list: getSizeBaseOptionList(size?.OptionList, size?.subparameterId, data?.PriceList[0].currencysymbol)
        }
...
export function getSizeBaseOptionList(PriceList: any, subparameterId: any, currencysymbol: any) {
    function isUnique(item: any, index: any, array: any) {
        return (
            index === array.findIndex((obj: any) =>
                obj.parameterId === item.parameterId
            )
        );
    }
...
function getOptionBaseType(PriceList: any, option: any, currencysymbol: any) {
    const filteredSubOptionList = PriceList.filter((suboption: any) => suboption?.parameterId === option?.parameterId)
...
export const generateTableName = (restaurantId: any, locationId: any, env: any) => {
    switch (env) {
        case "development":
...
export function getCustomerDetails(user: any, tableDetail: any, serverId: any = 0) {
...
export const calculateFinalCount = (subOptionList: any, selectedOption: any) => {
    let finalcount = 0;
    var toppingcount = subOptionList?.filter((x: any) => x.subOptionselected === true);
    toppingcount?.map((tc: any) => {
        ...
        var calculatedtopvalue =
            (selectedOption.isHalfPizza === true && (tc.pizzaside === "L" || tc.pizzaside === "R")) ? topvalue *
                (tc.halfPizzaPriceToppingPercentage === "" || parseInt(tc.halfPizzaPriceToppingPercentage) === 0
                    ? 1 : parseInt(tc.halfPizzaPriceToppingPercentage) / 100) : topvalue;
        finalcount = finalcount + tc.subOptionToppingQuantity * calculatedtopvalue;
    });
    return finalcount;
}

export const calculateFinalCountWithPaid = (subOptionList: any, selectedOption: any) => {
    let finalcount = 0;

    const toppingcount = subOptionList?.filter((x: any) => x.subOptionselected === true);

    toppingcount?.forEach((tc: any) => {
        const topvalue = tc.toppingValue === "" || parseInt(tc.toppingValue) === 0
            ? 1
            : parseInt(tc.toppingValue);

        const calculatedtopvalue =
            (selectedOption.isHalfPizza === true && (tc.pizzaside === "L" || tc.pizzaside === "R"))
                ? topvalue *
                (tc.halfPizzaPriceToppingPercentage === "" || parseInt(tc.halfPizzaPriceToppingPercentage) === 0
                    ? 1
...
export const calculateFinalCountTable = (subOptionList: any, selectedOption: any) => {
    let finalcount = 0;
    var toppingcount = subOptionList?.filter((x: any) => x.subOptionselected === true);
    toppingcount?.map((tc: any) => {
        var topvalue = tc.toppingValue === "" || parseInt(tc.toppingValue) === 0 ? 1 : parseInt(tc.toppingValue);
        var calculatedtopvalue =
            (selectedOption.isHalfPizza === true && (tc.pizzaside === "L" || tc.pizzaside === "R")) ? topvalue *
                (tc.halfpizzaprice === "" || parseInt(tc.halfpizzaprice) === 0
                    ? 1 : parseInt(tc.halfpizzaprice) / 100) : topvalue;
        finalcount = finalcount + tc.subOptionToppingQuantity * calculatedtopvalue;
    });
    return finalcount;
}
...
export const convertOptionToStrListTo = (...optionList: any) => {
    const optionStrList: any = []
    optionList?.map((item: any) => {
        const str = item?.reduce((acc: any, cur: any, index: any) => ` ${acc += `${cur.toppingquantity + cur.paidQty}x ${cur.type}${index === (item.length - 1) ? "" : ","}${" "}`}`, "")
        optionStrList.push(str);
    })
    return optionStrList;
}
...
export function checkOsType() {
    ...
    if (navigator.userAgent.indexOf("Win") != -1) type =
        OS_TYPE.OTHER.value
    // "Windows OS";
    if (navigator.userAgent.indexOf("Mac") != -1) type =
        OS_TYPE.APPLE.value;
    // "Macintosh";
    if (navigator.userAgent.indexOf("Linux") != -1) type =
        OS_TYPE.OTHER.value
    // "Linux OS";
    if (navigator.userAgent.indexOf("Android") != -1) type =
        OS_TYPE.ANDROID.value;
    if (navigator.userAgent.indexOf("like Mac") != -1) type =
        OS_TYPE.APPLE.value;
    // "iOS";
...
export const getDependentParentQty = (cartItems: any, item: any, index: any) => {
    let qty = 0
...
export const checkDisableWindow = (timeWindow: any, enableFutureOrdering: any, day: any) => {
    let isEnable = true
    if ((timeWindow && timeWindow.length === 0 && !enableFutureOrdering)) {
        isEnable = false
    }
    if (enableFutureOrdering && day === "Today" && timeWindow && timeWindow.length === 0) {
        isEnable = false
    }
...
export const calculateNettotal = (lstcarttopping: any, selectedsize: any, quantity: any, menuItemDetail: any) => {
    const selectedTopping = Object.keys(menuItemDetail).length > 0 && menuItemDetail?.topping?.find(
    ...
    if (lstcarttopping && lstcarttopping.length > 0) {
        lstcarttopping.forEach((data: any) => {
            ...
            if (paidQty > 0) {
                const unitPrice = (data.pizzaside === "L" || data.pizzaside === "R")
                    ? parseFloat((data.price * 0.5).toFixed(2))
                    : parseFloat(data.price);

                fsum += unitPrice * paidQty; // only charge for paidQty
            } else if ((paidQty === 0 && selectedOption.freeToppingsCount === 0) || !selectedOption?.multipleSelectStatus) {
    ...
    let total = (selectedsize && selectedsize.length > 0)
