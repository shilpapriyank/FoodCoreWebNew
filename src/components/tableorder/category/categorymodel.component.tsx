...
const CategoryModalComponent: React.FC = () => {
    ...
    const locationFullLink = `/${selectedTheme.url}/${restaurantinfo.restaurantURL}/${restaurantinfo?.defaultLocation?.locationURL.trim()}`;
    ...
    const handleClickCat = useCallback((cat: CategoryItem) => {
        if (cat) {
            ...
            router.push(`${locationFullLink}/${cat.categoryslug}?${PAGES.TABLENO}=${tableorder.tablenumber}`)
                        ...
                        <div className="text">
                            <ul className="menu-list-popup">
                                {main?.maincategoryList?.map((cat: CategoryItem) => (
                                    <li
                                        key={cat.catId}
                                        className={selectedCatId === cat?.catId ? 'color-orange cursor-pointer' : 'cursor-pointer'}
