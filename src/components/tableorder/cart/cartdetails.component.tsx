...
const CartDetailsComponent: React.FC<CartDetailsProps> = ({ handleClose }) => {
  const { menuitem, restaurantinfo, tableorder, userinfo, main } =
    useReduxData();
  const {
    getCartItems,
    incrementQty,
  ...
  const handleClickEdit = async (id: string) => {
    ...
    if (response) {
      const editMenuItem = response.find(
        (item: any) => item.menuitemId === editItem.menuitemId
      );
      ...
      const menuItemCategory = main?.maincategoryList?.find(
        (item: any) => item?.catId === editItem.catId
      );
  ...
  const handleClickAddToKitchenCopy = () => {
    TableOrderServices.sendTablePushnotification(
      ...
      if (res?.status === API_RESPONSE_STATUS.SUCCESS) {
        ...
        updates[
          `${collectionName}/${tableorder?.tabledetail?.tableId}/isToSetCurrentDeviceTag`
        ] = false;
          ...
          <div className="sticky-cart">
            <div className="d-flex align-items-center">
              ...
              <div className="add-to-kitchen-btn">
                <a
                  onClick={handleClickAddToKitchenCopy}
