...
const CategorySidebar: React.FC = () => {
    ...
    const [isOpenChangeLocation, setIsOpenChangeLocation] = useState<boolean>(false);
            ...
            <div className="col-lg-3 col-md-4 col-12 order-1 order-lg-2 order-md-2 d-none d-md-block scrollable-section">
                <div className="sidebar">
                    ...
                    <div className="card totalbox">
                        ...
                        {cart?.cartitemcount > 0 ? <a className="btn-default w-100" onClick={handleClick}> Go To Cart<i className="fa fa-angle-right" />{" "} </a>
                            : <a className="btn-default w-100 greyColor"> Go To Cart <i className="fa fa-angle-right" />{" "} </a>}
                    </div>
                </div>
            </div>

            {(restaurantinfo?.displayLocationPopUpChange && isOpenChangeLocation) &&
                <CommonModal title="Confirm Location"
                    isButton={true} btn1Name='Confirm' handleToggle={handleToggle} isOpenModal={isOpenChangeLocation} isbtn2={true} btn2Name="No change" handleClickBtn2={handleClickBtn2} handleClickBtn1={handleClickOk} isChild={true}>
                    <><h6 className=' mt-3 fs-6 text-center'>You have selected <span className='text-dynamic'>{restaurantinfo?.defaultLocation?.locationName}</span> location.</h6></>
                </CommonModal>
            }
        </>
    );
