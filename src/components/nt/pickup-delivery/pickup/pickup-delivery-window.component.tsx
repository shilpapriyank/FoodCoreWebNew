...
const PickupdeliveryWindowTime: React.FC<Props> = ({
  ...
  let isAlldeliveryWindowClose =
    deliveryWindow?.length > 0 &&
    deliveryWindow.every((time) => time.isClosed === true);
  let isAllPickupWindowClose =
    pickupWindow?.length > 0 &&
    pickupWindow.every((time) => time.isClosed === true);
  ...
  return (
    <div className="row mt-4">
      {isDisableAsapLaterOn && (
        <div className="row mt-2 mb-5">
          <p className="color-red text-center">
            Sorry we are closed for the day.Hope to see you soon
          </p>
        </div>
      )}
      ...
      !isAlldeliveryWindowClose ? (
        <div className="col-lg-6 col-md-6 col-6">
          {(isDeliveryAsap === true || isDeliveryPickupTime === true) && (
            <>
              <h6 className="color-gray">
                <i className="fa fa-clock" />
                &nbsp;{DeliveryConfirmationMessage.DELIVERY_WINDOWS}
              </h6>
              {isDeliveryWindowAvailable ? (
                <>
                  {deliveryWindow &&
                    deliveryWindow.map((time, index) => {
                      if (!time.isClosed) {
      ...
      pickupWindow &&
      pickupWindow?.length > 0 &&
      !isAllPickupWindowClose ? (
        <div className="col-lg-6 col-md-6 col-6">
          {(isTakeOutPickupTime === true || isTakeOutAsap === true) && (
            <>
              <h6 className="color-gray">
                <i className="fa fa-clock" />
                &nbsp;{PickUpConfirmationMessage.PICKUP_WINDOWS}
              </h6>
              {isPickupWindowAvailable ? (
                <>
                  {pickupWindow &&
                    pickupWindow.map((time, index) => {
                      if (!time.isClosed) {
                        return <div key={index}>{time.time} </div>;
                      } else {
                        return <></>;
