...
> = ({
  ...
  clearMeaage,
    ...
    const lastPickupTIme = pickupWindow && pickupWindow?.length > 0 && pickupWindow?.[pickupWindow?.length - 1];
    const lastDeliveryTime = deliveryWindow && deliveryWindow?.length > 0 && deliveryWindow?.[deliveryWindow?.length - 1];
    ...
    const [activeButtonClass, setActiveButtonClass] = useState<"" | "asap" | "lateron">
    ...
    return (
      <>
        ...
        >
          <div
            className={`modal-dialog modal-dialog-centered ${false ? "modal-dialog-scrollable" : ""
              }`}
          >
            {load && (
