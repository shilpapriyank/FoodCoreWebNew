...
import { emptyordertime, isasap, setDeliveryRequestId, setordertime } from '../../../../redux/order/order.slice'
...
import { DELIVERYSERVICES, GetThemeDetails, ORDERTYPE, ORDER_TYPE, checkWindowTimeExpires, getAsapLaterOnState } from '../../common/utility'
...
const PickupDeliveryTimeSelectPopup: React.FC<PickupDeliveryTimeSelectPopupProps> = ({
    isOpenModal,
    ...
    const { restaurantinfo, sessionid, restaurant, selecteddelivery, userinfo, order, main, deliveryaddress } = useReduxData()
    ...
    let locationFullLink = "/" + selectedTheme.url + "/" + dynamic + "/" + location + "/";
    ...
    const pickupordelivery = selecteddelivery?.pickupordelivery ? Object.keys(selecteddelivery?.pickupordelivery).length > 0 ? selecteddelivery?.pickupordelivery : "" : "";
    ...
    const lastPickupTIme = pickupWindow?.length > 0 && pickupWindow?.[pickupWindow?.length - 1]
    const lastDeliveryTime = deliveryWindow?.length > 0 && deliveryWindow?.[deliveryWindow?.length - 1]
    ...
    let selectedAsap = (clearData === false && order.isasap === true) ? true : false;
    let selectedLateron = (clearData === false && order.isasap === false) ? true : false;
    const [isAsap, setisAsap] = useState<boolean>(selectedAsap)
    const [isLaterOn, setisLaterOn] = useState<boolean>(selectedLateron)
    const [activeButtonClass, setActiveButtonClass] = useState<'' | 'asap' | 'lateron'>(clearData === false ? order.isasap === true ? "asap" : "lateron" : "");
    ...
    const [defaultLoactionId, setdefaultLoactionId] = useState<number | null>(restaurantinfo.defaultlocationId);
    var ordertype = selecteddelivery.pickupordelivery === ORDERTYPE.Delivery ? 2 : 1;
    let selectedAddress = userinfo === null ? deliveryaddress?.tempDeliveryAddress : selecteddelivery?.selecteddeliveryaddress;
    const { deliveryService } = restaurantinfo?.defaultLocation
    let hour;
    let minute;
    let meridiem;
    load = true;
    ...
    const [isDeliveryWindowAvailable, setisDeliveryWindowAvailable] = useState(true);
    ...
    useEffect(() => {
        ...
        RestaurantsServices.getCurrentTime(restaurantinfo?.restaurantId, restaurantinfo?.defaultlocationId).then((res) => {
            const responseTime = res?.datetime.split(" ").reverse()[0]
            if (pickupWindow.length > 0) {
                const isValid = checkWindowTimeExpires(pickupEndTime, responseTime, lastPickupTIme?.isLastOrder, isAsap)
                setisPickupWindowAvailable(isValid)
            }
            if (deliveryEndTime.length > 0) {
                const isValid = checkWindowTimeExpires(deliveryEndTime, responseTime, lastDeliveryTime?.isLastOrder, isAsap)
    ...
    useEffect(() => {
        if (document.getElementById("datetimepicker4") !== null && isLaterOn === true &&
            document.getElementById("datetimepicker4") !== undefined && isTimeLoad && !defaultLocation.isOrderingDisable) {
            let hour = Meridiem === "AM" ? parseInt(Hour) : parseInt(Hour) + 12 === 24 ? 12 : parseInt(Hour) + 12;
            let Time = `${hour}:${parseInt(Minute)}`;

            ($('#datetimepicker4') as any)?.datetimepicker({
                format: 'LT',
                inline: true,
            }
    ...
    }, [document.getElementById("datetimepicker4"), isLaterOn, isTimeLoad, Hour, Minute])
    ...
    const handleAsapClick = () => {
        if (ordertype === ORDER_TYPE.DELIVERY.value && deliveryService === DELIVERYSERVICES.UBEREATS && selectedAddress === null) {
            handleNotify(ERRORMESSAGE.SELECT_ADDRESS, ToasterPositions.TopRight, ToasterTypes.Warning);
            return;
        }
    ...
    const handleLaterOnConfirmSelectionClick = () => {
        let datepickerwidge = document.querySelectorAll(".bootstrap-datetimepicker-widget")
    ...
    const handlesave = async (hour: string, minute: string, meridiem: string): Promise<void> => {
        try {
    ...
    return (
        <>
            <div className={`modal fade modal-your-order address-modal ${isOpenModal ? 'show d-block' : ''}`} style={{ display: 'block' }} id="order-time-modal" aria-labelledby="order-time-modal-Label" aria-hidden="true" >
                <div className={`modal-dialog modal-dialog-centered ${false ? "modal-dialog-scrollable" : ""}`}>
                    {load &&
                        <div className="modal-content">
                            <a>
                                <h5 className="modal-title" id="login-modal-Label"> {DELIVERYPAGEMESSAGE.CHOOSE_YOUR_TIME} {pickupordelivery !== "" && `for ${pickupordelivery}`}</h5>
                                <button type="button" className="btn-close" onClick={() => handleToggleTimingModal(false)} aria-label="Close" />
                            </a>
                            <div className="modal-body">
                                ...
                                {((selecteddelivery.pickupordelivery === ORDER_TYPE.DELIVERY.text && isDeliveryWindowAvailable) || (selecteddelivery.pickupordelivery === ORDER_TYPE.PICKUP.text && isPickupWindowAvailable) || selecteddelivery.pickupordelivery === "") && <> {
                                    isAsap && <>
                                        {/* hide-this */}
                                        <form className="asap-date-form  my-3">
                                            <div className="row justify-content-center">
                                                <div className="col-lg-12 col-sm-12 col-12 text-center">
                                                    {timeOrErrorMessage !== "" && <h4 style={{ color: "red" }}>{timeOrErrorMessage}</h4>}
                                                    {((timeOrErrorMessage === "" && orderTime !== "") || selecetdtime !== "") &&
                                                        <h4>{orderTime === "" ? selecetdtime : orderTime},&nbsp; Today,&nbsp; {currentDate?.getDate()}&nbsp;{MonthList(currentDate?.getMonth() ?? 0)}</h4>
                                                    }
                                                    {
                                                        (isConfirmDisable || defaultLocation.isOrderingDisable) ? <a className=" greyColor non-cursor btn-default mt-3 px-5">{DELIVERYPAGEMESSAGE.CONFIRM_SELECTION}</a>
                                                            : <a className="btn-default mt-3 px-5" onClick={handleAsapConfirmClick}> {DELIVERYPAGEMESSAGE.CONFIRM_SELECTION}</a>
                                                    }
                                                </div>
                                            </div>
                                        </form>
                                ...
                                }
                                    {
                                        ...
                                        <form className="later-date-form ">
                                            <div className="row">
                                                <div className="col-lg-12 col-sm-12 col-12 text-center" id="datepicker" >
                                                    <h4>{DELIVERYPAGEMESSAGE.SELECT_TIME}</h4>
                                                    <div className="d-block mx-auto" id="datetimepicker4" />
                                                </div>
                                                {timeOrErrorMessage !== "" && <h6 className="text-center error">{timeOrErrorMessage}</h6>}
                                                {successMessage !== "" && <h6 style={{ color: "green" }} className="text-center">{successMessage}</h6>}
                                                {
                                                    (isConfirmDisable || defaultLocation.isOrderingDisable) ?
                                                        (<div className="col-lg-12 col-md-12 col-12 text-center">
                                                            <a className=" greyColor opacity-50 pe-none non-cursor btn-default mt-3 px-5">{DELIVERYPAGEMESSAGE.CONFIRM_SELECTION} </a>
                                                        </div>)
                                                        :
                                                        (<div className="col-lg-12 col-md-12 col-12 text-center">
                                                            <a className="btn-default px-5" onClick={handleLaterOnConfirmSelectionClick} > {DELIVERYPAGEMESSAGE.CONFIRM_SELECTION} </a>
                                                        </div>)
                                                }
                                            </div>
                                        </form>
                                ...
                                <PickupdeliveryWindowTime
                                    ...
                                    isDeliveryAsap={isDeliveryAsap}
                                    isTakeOutAsap={isTakeOutAsap}
                                    isDeliveryPickupTime={isDeliveryPickupTime}
                                    isTakeOutPickupTime={isTakeOutPickupTime} // Add this
