...
function DeliveryAddressHoc<P extends DeliveryAddressHocProps>(
    ...
    const NewComponent: React.FC<Omit<P, keyof DeliveryAddressHocProps>> = (props) => {
        ...
        const handleClickSelectDeliveryAddress = useCallback(
            (address: DeliveryAddressItem) => {
                if (address.deliveryaddressId !== deliveryaddress?.addressId?.customerAddressId) {
        ...
        const deleteAddressString = useMemo(() => {
            const selectedDeleteAddress = addresslist && addresslist?.find((ad) => ad.deliveryaddressId === deleteAddressId)
            return <>
            ...
            <>
                {addresslist.map((address) => (
                    <OriginalComponent
                        ...
                        isChecked={address?.deliveryaddressId === selecteddeliveryaddres?.deliveryaddressId}
                        address={address}
                    />
                ))}
                {openDelete && (
