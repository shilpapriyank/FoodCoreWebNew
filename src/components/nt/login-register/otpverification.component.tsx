...
import { GetThemeDetails, closeModal, unFormatePhoneNumber } from '../../common/utility';
...
const OtpVerificationComponent: React.FC<Props> = ({
    ...
    let otpTime = convertSecondToMinute(parseInt(process.env.NEXT_PUBLIC_OTP_DURATION || '60'));
    ...
    const registerAddress = Object.keys(deliveryaddress.registeraddress || {}).length > 0 ? deliveryaddress.registeraddress : deliveryaddress.tempDeliveryAddress;
    ...
    const handleClickValidateOTP = (isVerifiedPhone: boolean) => {
        ...
        if (restaurantinfo.smsapigateway === SMS_API_TYPE.FIREBASE.value && restaurantinfo.enableotpauthentication === true) {
            CustomerServices.updateVerifiedPhoneNumber(updateObj as any).then((res: any) => {
                if (res !== undefined) {
                    dispatch({
    ...
    const registerUser = () => {
        ...
        var requesturl = window.location.origin + "/" + restaurantUrl + "/confirmation?code=";
        RegisterServices.registerUserWithAddress(userModel, addressmodel, restaurantinfo.defaultlocationId, restaurantinfo.restaurantId, requesturl)
            .then((response: any) => {
                if (response && response != null) {
                    if (response.message !== null && response.status == 2) {
                        ...
                        handleNotify(`Welcome ${userModel.firstname} `, ToasterPositions.TopRight, ToasterTypes.Success);
    ...
    const handleSendOTP = () => {
        let unformatedphone = userModel.phone.replace('(', '').replace(')', '').replace('-', '').replace(' ', '');
        if (restaurantinfo.smsapigateway === SMS_API_TYPE.FIREBASE.value && restaurantinfo.enableotpauthentication === true) {
            handleSendOtpHook(dialCode.toString(), unformatedphone).then((res) => {
                document.getElementById("0")?.focus()
    ...
    const handleValidateOTP = (OTP: string) => {
        ...
        if (restaurantinfo.smsapigateway === SMS_API_TYPE.FIREBASE.value && restaurantinfo.enableotpauthentication === true) {
            handleValidateOTPHook(code as any).then((res: any) => {
    ...
    return (
        <>
            <div className={`modal modal-your-order loginmodal fade ${isOpenModal ? 'show d-block' : ''}`} id="otp-modal" tabIndex={-1} aria-labelledby="otp-modall-Label" aria-hidden="true">
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content">
                        <h5 className="modal-title" id="login-modal-Label">Verification required</h5>
                        {
                            isNotValidateOtp === true ? <button type="button" onClick={handleClose} className="btn-close" aria-label="Close" /> :
                                <button type="button" onClick={() => handleToggle(false, userModel)} aria-label="Close" />
                        }
                        <div className="modal-body">
                            <p>{`Please enter 6 digit verification code sent ${dialCode}${unPhormatedPhone}`} </p>
                            <div className='row d-flex justify-content-center'>
                                <OtpInput value={otp} valueLength={6} onChange={onChange} />
                            </div>
                            <div >
                                <h3 className='text-center color-gray'>{`${minutes < 10 ? `0${minutes}` : minutes}`}:{`${seconds < 10 ? `0${seconds}` : seconds}`}</h3>
                            </div>
                            {(restaurantinfo.smsapigateway === SMS_API_TYPE.FIREBASE.value && restaurantinfo.enableotpauthentication === true) &&
                                <div className="col-lg-12 col-sm-12 col-xs-12 d-flex justify-content-center mt-2">
                                    <div style={{ width: "32% !important", border: "none !important" }} id="recaptcha-id" data-container="recaptcha-container"></div>
                                </div>
                            }
                        </div>
                        <div className="modal-footer">
                            <div className="row w-100 ms-auto me-auto">
                                <div className="col-lg-6 text-center col-md-6 col-12">
                                    {
                                        resendDisable ? <a className="btn-default w-100 button-disable pe-none opacity-50 " aria-disabled="true"  >Resend</a> :
                                            <a className="btn-default w-100" onClick={handleSendOTP} >Resend</a>
                                    }
                                </div>
                                <div className="col-lg-6 text-center col-md-6 col-12">
                                    {otp && otp?.length === 6 ?
                                        <a className="btn-default w-100" onClick={() => handleValidateOTP(otp)} >Submit</a>
                                        :
                                        <a className="btn-default button-disable w-100 pe-none opacity-50 " aria-disabled="true" >Submit</a>
                                    }
                                </div>
                            </div>
                        </div>
