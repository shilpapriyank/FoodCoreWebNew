...
const Register: React.FC<{
    ...
    handleToggleAddAddressModal
        ...
        const [, setisDisable] = useState<boolean>(false);
        const [, setisAddAddress] = useState<boolean>(false)
        const b2b = restaurantinfo?.defaultLocation?.b2btype
        let registerAddressdata = deliveryaddress.registeraddress;
        const [errors, setErrors] = useState<Record<string, string>>({});
        const [, setErrorMessage] = useState(null);
        const [, setuserExistError] = useState<string>("")
        const [, setIsShowReSend] = useState<boolean>(false);
