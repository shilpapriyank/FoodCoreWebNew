...
const FutureDayComponent: React.FC<FutureDayComponentProps> = ({
    ...
    return (
        ...
        >
            {
                enablefutureordering && futureDateList?.map((day, index) => {
                    const isClose = ordertype === ORDER_TYPE.DELIVERY.value ? (day?.deliveryStatus === "Closed") : (day?.takeoutStatus === "Closed");
                    return <Fragment key={index}>
                        <div>
                            ...
                            >
                                <div className={`rounded border border-2 shadow p-3 pointer-cursor ${((day?.futureDay === selectedDate) || (day?.futureDay === order?.futureOrderDay?.futureDay)) ? "border-dynamic" : ""} `}>
            ...
            <button onClick={handleSlideNext} className='swiper-button-next bg-white op-1'></button>
            <button onClick={handleSlidePrev} className='swiper-button-prev bg-white op-1'></button>
            <br></br>
        </Swiper >
    );
};
